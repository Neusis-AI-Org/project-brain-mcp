...
func ensureDockerImageBuilt(t *testing.T) {
	buildOnce.Do(func() {
		...
		cmd := exec.Command("docker", "build", "-t", "github/e2e-mcp-project-brain", ".")
...
func setupMCPClient(t *testing.T, options ...clientOption) *mcp.ClientSession {
	...
	if os.Getenv("GITHUB_MCP_SERVER_E2E_DEBUG") == "" {
		...
		// Add the image name
		args = append(args, "github/e2e-mcp-project-brain")
...
func TestTags(t *testing.T) {
	...
	repoName := fmt.Sprintf("mcp-project-brain-e2e-%s-%d", t.Name(), time.Now().UnixMilli())
